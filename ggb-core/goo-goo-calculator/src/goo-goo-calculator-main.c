...
static const HELP_text help[] = {
    [EN_US] = {.unknown = "Unknown parameter",
...
static const VERSION_text version[] = {
    [EN_US] = {.tool_name = "GOO-GOO Calculator (GGC)", .tool_version = "GGC Version"},

    [ZH_CN] = {.tool_name = "GOO-GOO 计算器 (GGC)", .tool_version = "GGC 版本"}};
...
static void long_unknow(unknown_var **var, const CLIarg arg, const int which_arg_unknow)
{
	...
	(*var)->unknown_text =
	    malloc(strlen(arg.argv[which_arg_unknow]) + 1); // The reason for adding 1 is to accommodate '\0'
 ...
 */
static void short_unknow(unknown_var **var, const CLIarg arg, const int which_arg_unknow, const int which_unknow,
                         const size_t len)
{
	(*var)->unknown_switch = true;
	if (len < 2)
